...
const paymentForm = ref({ payment_type: 1, amount: 0, crypto_quantity: null, crypto_rate: null, tx_link: '', fund_storage_id: null, team_id: null, description: '', payment_date: today })
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await fetch(`/api/intermediary-cases/${interId}?${params}`, { headers })
    if (res.ok) {
      const data = await res.json()
      inter.value = data.intermediary
...
const fetchAllPayments = async () => {
  ...
  try {
    ...
    const res = await fetch(`/api/intermediary-cases/${interId}/payments?${params}`, { headers })
    if (res.ok) {
      const data = await res.json()
...
onMounted(() => { fetchData(); fetchAllPayments(); fetchFundStorages() })
...
const addPayment = async () => {
  const res = await fetch(`/api/intermediary-cases/${interId}/payments`, { method: 'POST', headers, body: JSON.stringify(paymentForm.value) })
  const data = await res.json()
  if (res.ok) {
    showPaymentDialog.value = false
    paymentForm.value = { payment_type: 1, amount: 0, crypto_quantity: null, crypto_rate: null, tx_link: '', fund_storage_id: null, team_id: null, description: '', payment_date: today }
...
const openDayDetail = async (day) => {
  ...
  try {
    const res = await fetch(`/api/intermediary-cases/${interId}/payments?date=${day.date}`, { headers })
    if (res.ok) {
      const data = await res.json()
...
const deletePayment = async (paymentId) => {
  ...
  const res = await fetch(`/api/intermediary-cases/${interId}/payments/${paymentId}`, { method: 'DELETE', headers })
...
const paymentTypeLabel = (type) => {
  if (type === 1) return t('merchant_case.type_tl')
  if (type === 2) return t('merchant_case.type_crypto')
  return t('intermediary_case.team_offset')
}
...
<template>
      ...
      <VCard :loading="loading">
        <VCardText class="d-flex align-center justify-space-between flex-wrap gap-4">
          <div class="d-flex align-center gap-3">
            <VBtn icon variant="text" size="small" @click="router.push('/intermediary-cases')">
              <VIcon icon="tabler-arrow-left" />
            </VBtn>
            <div>
              <h4 class="text-h4 font-weight-bold">{{ inter.name }}</h4>
              <VChip :color="inter.type === 1 ? 'primary' : 'warning'" label size="x-small">
                {{ inter.type === 1 ? t('intermediary.paylira_type') : t('intermediary.merchant_type') }}
              </VChip>
            </div>
          </div>
        ...
        <VTable class="text-no-wrap">
          <thead>
            <tr>
              ...
              <th class="text-end">{{ t('intermediary_case.daily_commission') }}</th>
          ...
          <tbody>
            <tr v-for="day in dailyCases" :key="day.date" class="cursor-pointer" :style="day.is_today ? 'background: rgba(var(--v-theme-primary), 0.08)' : ''" @click="openDayDetail(day)">
              ...
              <td class="text-end text-success">{{ formatMoney(day.daily_commission) }}</td>
        ...
        <VTable class="text-no-wrap" density="compact">
          ...
          <tbody>
            <tr v-for="p in allPayments" :key="p.id">
              ...
              <td>
                <VChip :color="paymentTypeColor(p.payment_type)" label size="x-small">
                  {{ paymentTypeLabel(p.payment_type) }}
                </VChip>
                <span v-if="p.team_name" class="text-caption text-medium-emphasis ms-1">{{ p.team_name }}</span>
              </td>
              ...
              <td class="text-end">
                <VBtn v-if="isPaymentToday(p.created_at)" icon size="x-small" variant="text" color="error" @click="deletePaymentFromList(p.id)">
    ...
    <VCard :title="t('merchant_case.add_payment')">
      <VCardText>
        <VRadioGroup v-model="paymentForm.payment_type" inline class="mb-4">
          ...
          <VRadio :label="t('intermediary_case.team_offset')" :value="3" />
        </VRadioGroup>
        ...
        <template v-if="paymentForm.payment_type === 3">
          <VSelect v-model="paymentForm.team_id" :items="teams" item-title="name" item-value="id" :label="t('intermediary_case.select_team')" class="mb-4" />
  ...
  <!-- Gün Detay -->
  <VDialog v-model="showDayDetailDialog" max-width="650">
    <VCard :title="`${inter.name} — ${selectedDay ? formatDate(selectedDay.date) : ''}`" :loading="dayPaymentsLoading">
      <VCardText>
          ...
          <VTable density="compact" class="text-no-wrap">
            ...
            <tbody>
              <tr v-for="p in dayPayments" :key="p.id">
                <td>
                  <VChip :color="paymentTypeColor(p.payment_type)" label size="x-small">{{ paymentTypeLabel(p.payment_type) }}</VChip>
                  <span v-if="p.team_name" class="text-caption text-medium-emphasis ms-1">{{ p.team_name }}</span>
                </td>
                <td class="text-end font-weight-medium">{{ formatMoney(p.amount) }}</td>
                <td class="text-body-2">
