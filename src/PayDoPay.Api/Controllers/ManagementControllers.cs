...
public class SettingsController : MgmtControllerBase
{
    ...
    public async Task<IActionResult> TestAnthropic([FromServices] PayDoPay.Application.Features.Receipts.IClaudeVisionService vision, CancellationToken ct)
    {
        var u = await CurrentUser.GetUserAsync(ct);
    ...
    public async Task<IActionResult> AnalyzeTest([FromBody] AnalyzeTestBody b, [FromServices] PayDoPay.Application.Features.Receipts.IClaudeVisionService vision, CancellationToken ct)
    {
        var u = await CurrentUser.GetUserAsync(ct);
        if (u is null) return Unauthorized(new { message = "Unauthenticated." });
        ...
        var expected = new PayDoPay.Application.Features.Receipts.ReceiptExpected(b.Amount ?? 0, b.Iban ?? "", b.Recipient ?? "");
